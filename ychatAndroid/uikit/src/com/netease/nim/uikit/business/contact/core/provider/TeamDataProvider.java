...
public class TeamDataProvider {
    ...
    private static int compareTeam(TeamContact lhs, TeamContact rhs) {
        ...
        return TextComparator.compareIgnoreCase(lhs.getTeam().getCreateTime(), rhs.getTeam().getCreateTime());
    }
     ...
     * * 成员数据查询
     */
    private static final List<AbsContactItem> query(TextQuery query) {
        List<AbsContactItem> contacts = new ArrayList<>();
        TeamProvider provider = NimUIKit.getTeamProvider();
        List<Team> teams = provider.getAllTeams();
        for (Team t : teams) {
            List<AbsContactItem> contactItems = TeamMemberDataProvider.provide(query, t.getId());
            if (contactItems != null && contactItems.size() > 0) {
                contacts.add(contactItems.get(0));
            }
        }
        return contacts;
    }
