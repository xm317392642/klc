...
public class TeamHelper {
     ...
     * @return
     */
    public static ContactSelectActivity.Option getCopySelectOption(String teamId, ArrayList<String> memberAccounts, String robotId) {
        ContactSelectActivity.Option option = new ContactSelectActivity.Option();
        option.type = ContactSelectActivity.ContactSelectType.TEAM_MEMBER;
        option.title = "选择复制的成员";
...
//        }
    ...
    public static boolean changeTeamInfo(String tid, String account) {
        boolean changeTeamInfo = false;
        Team team = NimUIKit.getTeamProvider().getTeamById(tid);
        if (team != null && team.getType() == TeamTypeEnum.Advanced) {
            TeamMember member = NimUIKit.getTeamProvider().getTeamMember(tid, account);
            if (member != null) {
                TeamUpdateModeEnum teamUpdateModeEnum = team.getTeamUpdateMode();
                if (teamUpdateModeEnum == TeamUpdateModeEnum.All) {
                    changeTeamInfo = true;
                } else {
    ...
    public static boolean isTeamMember(String tid, String account) {
        ...
        if (team != null && team.getType() == TeamTypeEnum.Advanced) {
            TeamMember member = NimUIKit.getTeamProvider().getTeamMember(tid, account);
            if (member != null && member.isInTeam()) {
                changeTeamInfo = true;
            }
        }
        return changeTeamInfo;
