...
public class MessageListPanelEx {
        ...
        public void onItemChildClick(IRecyclerView adapter2, View view, int position) {
            ...
            if (LinkElement.TYPE_URL.equals(element.getType())) {
                ...
                try {
                    ...
                    YchatToastUtils.showShort("路径错误");
                }
    ...
    public void scrollToPosition(IMMessage message) {
        uiHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    /**
     * 当用户清空消息记录了，隐藏x条消息的显示view
     */
    private void dismiss() {
        MessageFragment messageFragment = null;
        if (container.activity instanceof TeamMessageActivity) {
            TeamMessageActivity teamMessageActivity = (TeamMessageActivity) container.activity;
            messageFragment = (MessageFragment) teamMessageActivity.getSupportFragmentManager().getFragments().get(0);
        } else if (container.activity instanceof P2PMessageActivity) {
            P2PMessageActivity p2PMessageActivity = (P2PMessageActivity) container.activity;
            messageFragment = (MessageFragment) p2PMessageActivity.getSupportFragmentManager().getFragments().get(0);
        }
        messageFragment.leftToRightAnim();
    }
    ...
    private MessageListPanelHelper.LocalMessageObserver incomingLocalMessageObserver = new MessageListPanelHelper.LocalMessageObserver() {
        ...
        @Override
        public void onUpdateMessage(IMMessage message) {
            adapter.notifyDataSetChanged();
        }
    };
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private int getItemIndex(IMMessage message) {
        int index1 = Collections.binarySearch(items, message,
                new Comparator<IMMessage>() {
                    @Override
                    public int compare(IMMessage o1, IMMessage o2) {
                        if (o1.isTheSame(o2)) {
                            return 0;
    ...
    private int getItemIndex(String uuid) {
        ...
        for (int i = 0, j = size - 1; j > i; i++, j--) {
            IMMessage start = items.get(i);
            if (TextUtils.equals(start.getUuid(), uuid)) {
                return i;
            }
            IMMessage end = items.get(j);
            if (TextUtils.equals(end.getUuid(), uuid)) {
                return j;
            }
        }
        return -1;
...
//            });
        ...
        private void longClickItemEarPhoneMode(CustomAlertDialog alertDialog, MsgTypeEnum msgType) {
            ...
            alertDialog.addItem(content, new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                @Override
                public void onClick() {
                    YchatToastUtils.showShort(finalContent);
                    setEarPhoneMode(!UserPreferences.isEarPhoneModeEnable(), true);
                    simpleCallback.onResult(true, content, 200);
                }
            });
        }
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
                    NIMClient.getService(MsgService.class).revokeMessageEx(item, "撤回一条消息", payload).setCallback(new RequestCallback<Void>() {
                        ...
                        public void onFailed(int code) {
                            if (code == ResponseCode.RES_OVERDUE) {
                                YchatToastUtils.showShort(R.string.revoke_failed);
                            } else {
                                YchatToastUtils.showShort("revoke msg failed, code:" + code);
                            }
                        }
    ...
    public IMMessage getUserLastSendMessage() {
        IMMessage lastMessage = null;
        for (int i = items.size() - 1; i >= 0; i--) {
            IMMessage msg = items.get(i);
            if (msg != null && msg.getDirect() == MsgDirectionEnum.Out && msg.getMsgType() != MsgTypeEnum.tip && msg.getMsgType() != MsgTypeEnum.notification) {
                lastMessage = msg;
                break;
            }
        }
    ...
    private boolean sendMessageCheck(final IMMessage msg) {
        return msg != null
                && msg.getDirect() == MsgDirectionEnum.Out
                && msg.getMsgType() != MsgTypeEnum.tip
                && msg.getMsgType() != MsgTypeEnum.notification;
    ...
    private void doForwardMessage(final String sessionId, SessionTypeEnum sessionTypeEnum) {
        ...
        if (message == null) {
            YchatToastUtils.showShort("该类型不支持转发");
            return;
        }
