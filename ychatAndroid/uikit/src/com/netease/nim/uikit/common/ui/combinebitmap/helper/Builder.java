...
import com.netease.nim.uikit.common.ui.combinebitmap.layout.ILayoutManager;
...
import com.netease.nim.uikit.common.ui.combinebitmap.region.IRegionManager;
import com.netease.nim.uikit.common.ui.combinebitmap.region.WechatRegionManager;
...
public class Builder {
    ...
    public int placeholder = R.drawable.nim_avatar_default; // 获取图片失败时的默认图片
    ...
    public Builder setMembers(List<TeamMember> members) {
    ...
    private int getSubSize(int size, int gap, ILayoutManager layoutManager, int count) {
        int subSize = 0;
        if (layoutManager instanceof WechatLayoutManager) {
            if (count < 2) {
                subSize = size;
            } else if (count < 5) {
            ...
            throw new IllegalArgumentException("Must use WechatRegionManager!");
        }
        return subSize;
    }
