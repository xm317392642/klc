...
public class PickImageHelper {
    ...
    public static void pickImage(final Context context, final int requestCode, final PickImageOption option) {
        ...
        dialog.addItem(context.getString(R.string.cancel), new CustomAlertDialog
                .onSeparateItemClickListener() {
            @Override
            public void onClick() {
                dialog.dismiss();
     ...
     * 直接打开相册
     */
    public static void pickCode(final Context context, final int requestCode, final PickImageOption option) {
        if (context == null) {
            return;
        }
        int from = PickImageActivity.FROM_LOCAL;
        PickImageActivity.start((Activity) context, requestCode, from, option.outputPath, option.multiSelect, option.multiSelectMaxCount,
                false, option.crop, option.cropOutputImageWidth, option.cropOutputImageHeight);
    }
}
