...
public class SettingsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (viewHolder == null) {
            ...
            viewHolder.root = (RelativeLayout) convertView;
        ...
        } else if (item.getType() == SettingType.TYPE_SEPERATOR_TEXT) {
            updateSeperatorItemWithText(viewHolder, item);
        } else if (item.getType() == SettingType.TYPE_LINE) {
            addLineItem(viewHolder);
        } else if (item.getType() == SettingType.TYPE_TEXT_ARROW) {
            updateDefaultItemWithArrow(viewHolder, item);
        } else if (item.getType() == SettingType.TYPE_ARROW) {
            updateItemWithArrow(viewHolder, item);
        } else {
     ...
     * @param item
     */
    private void updateDefaultItemWithArrow(ViewHolder viewHolder, SettingTemplate item) {
        ViewGroup.LayoutParams lp = viewHolder.root.getLayoutParams();
        if (lp != null) {
            if (itemHeight > 0) {
                lp.height = itemHeight;
        ...
        RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) viewHolder.detailView.getLayoutParams();
        layoutParams.rightMargin = ScreenUtil.dip2px(22f);
        setTextView(viewHolder.titleView, item.getTitle());
        setTextView(viewHolder.detailView, item.getDetail());
     ...
     * @param item
     */
    private void updateItemWithArrow(ViewHolder viewHolder, SettingTemplate item) {
        ViewGroup.LayoutParams lp = viewHolder.root.getLayoutParams();
        if (lp != null) {
            if (itemHeight > 0) {
                lp.height = itemHeight;
     ...
     * 设置带文字的分离器
     */
    private void updateSeperatorItemWithText(ViewHolder viewHolder, SettingTemplate item) {
        ViewGroup.LayoutParams lp = viewHolder.root.getLayoutParams();
        if (lp != null) {
            viewHolder.root.setBackgroundColor(Color.TRANSPARENT);
            TextView textView = new TextView(context);
            LinearLayout.LayoutParams txParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            textView.setPadding(ScreenUtil.dip2px(14f),ScreenUtil.dip2px(5f),0,0);
            textView.setTextColor(ContextCompat.getColor(context, R.color.color_grey_999999));
            ...
            lp.height = ScreenUtil.dip2px(32f);
            viewHolder.root.setLayoutParams(lp);
        }
        viewHolder.headImageView.setVisibility(View.GONE);
        viewHolder.titleView.setVisibility(View.GONE);
    ...
    private void updateSeperatorItem(ViewHolder viewHolder) {
        ViewGroup.LayoutParams lp = viewHolder.root.getLayoutParams();
        if (lp != null) {
            lp.height = ScreenUtil.dip2px(10f);
            viewHolder.root.setLayoutParams(lp);
            viewHolder.root.setBackgroundColor(Color.TRANSPARENT);
        }
        viewHolder.headImageView.setVisibility(View.GONE);
