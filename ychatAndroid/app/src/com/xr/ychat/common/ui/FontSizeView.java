package com.xr.ychat.common.ui;
...
public class FontSizeView extends View {
    ...
    private int textStandardColor = Color.parseColor("#888888");
    //字体大小
    private int smallSize = 14;
    private int standerSize = 17;
...
//        mTextPaint.setStrokeWidth(ScreenUtil.dp2px(context, 1));
        ...
        mText2Paint.setColor(textStandardColor);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //画字
        canvas.drawText("A", points.get(0).x - textScaleX / 2+paddingLeft, height / 2 - 40+paddingTop, mTextPaint);

        //画字
        canvas.drawText("标准", points.get(1).x - text2ScaleX / 2+paddingLeft, height / 2 -80+paddingTop, mText2Paint);

        //画字
        canvas.drawText("A", points.get(points.size() - 1).x - text1ScaleX / 2+paddingRight, height / 2 - 40+paddingTop, mText1Paint);

        // 先画中间的横线
        canvas.drawLine(points.get(0).x+paddingLeft, height / 2+paddingTop, points.get(points.size() - 1).x +paddingLeft, height / 2+paddingTop, mLinePaint);


        // 绘制刻度
        for (Point point : points) {
            canvas.drawLine(point.x + 1+paddingLeft, height / 2 - 20+paddingTop, point.x + 1 +paddingLeft, height / 2 + 20+paddingTop, mLinePaint);
        }
        ...
        // 实体圆
        canvas.drawCircle(currentX + 1+paddingLeft, circleY+paddingTop, circleRadius, mCirclePaint);
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        currentX = event.getX()-paddingLeft;
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                //回到最近的一个刻度点
                Point targetPoint2 = getNearestPoint(currentX);
